...
def n_quartil(data_list, n):
    if n not in [1,2,3]:
        raise Exception('N argument must be 1, 2, or 3 for n_quartil operation')
...
def n_percentil(data_list, n):
    ...
    if n < 1 or n > 99:
        raise Exception('N argument must be between 1 and 99 for n_percentil operation')
...
if __name__ == "__main__":
    ...
    parser.add_argument('path', type=str, action='store', help='The path with file of numbers')
    parser.add_argument('-n', type=int, action='store', help='N parameter for quartil and percentil')
